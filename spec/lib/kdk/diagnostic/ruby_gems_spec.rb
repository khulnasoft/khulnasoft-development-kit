...
RSpec.describe KDK::Diagnostic::RubyGems do
  ...
  def stub_bundle_check(success)
    stub_shellout("/home/git/kdk/support/tool-version-manager-exec . bundle exec bundle check", success)
  end

  def stub_gem_installed(name, success)
    stub_shellout("/home/git/kdk/support/tool-version-manager-exec . bundle exec gem list -i #{name}", success)
  end

  def stub_gem_loads_ok(name, success)
    gem_name = KDK::Diagnostic::RubyGems::GEM_REQUIRE_MAPPING[name]
    stub_shellout("/home/git/kdk/support/tool-version-manager-exec . bundle exec ruby -r #{gem_name} -e 'nil'", success)
  end
