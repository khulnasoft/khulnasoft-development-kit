...
module KDK
  module TaskHelpers
    ...
    class RailsMigration
      ...
      def migrate_tasks
        tasks = {}
        tasks['rails'] = MAIN_TASKS unless geo_secondary? || in_recovery? || main_already_migrated?
      ...
      def rake(tasks)
        project = KDK::Project::Base.new('khulnasoft', KDK.config.khulnasoft.dir, 'master')

        project.with_stashed do
      ...
      def main_already_migrated?
        Diagnostic::PendingMigrations.new.success?
      end
