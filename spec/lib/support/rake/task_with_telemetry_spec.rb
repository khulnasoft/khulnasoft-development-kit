...
RSpec.describe Support::Rake::TaskWithTelemetry do
  ...
  describe '#execute' do
    ...
    before do
      allow(KDK::Telemetry).to receive(:telemetry_enabled?).and_return(telemetry)
    end
    ...
    context 'with telemetry enabled' do
      ...
      it 'sends telemetry' do
        ...
        expect(KDK::Telemetry).to receive(:send_telemetry).with(true, 'rake test-task', hash_including(
          :duration,
          environment: KDK::Telemetry.environment,
          platform: KDK::Machine.platform,
          architecture: KDK::Machine.architecture
        ))
        expect(KDK::Telemetry).to receive(:flush_events)
      ...
      context 'when the command fails' do
        it 'sends telemetry and propagates the error' do
          ...
          expect(KDK::Telemetry).to receive(:send_telemetry).with(false, 'rake test-task', hash_including(
            :duration,
            environment: KDK::Telemetry.environment,
            platform: KDK::Machine.platform,
            architecture: KDK::Machine.architecture
