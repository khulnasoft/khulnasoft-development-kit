...
RSpec.describe KDK::Command::Update do
  ...
  before do
    ...
    # Stub monotonic clock to return 0 for both start and end
  ...
  describe '#run', :hide_output do
    ...
    context 'when self-update is enabled' do
      it 'runs self-update and update' do
        ...
        expect(KDK::Output).to receive(:success).with('Successfully updated in 0ms!')
    ...
    context 'when `kdk` command no longer available after update' do
      ...
      it 'prints an error' do
        ...
        expect(subject).to receive(:run_rake).with(:reconfigure).and_return(true)
        expect(KDK::Telemetry).to receive(:capture_exception).with(described_class::KdkNotFoundError.new('`kdk` command is no longer available'))
        expect(KDK::Output).to receive(:error).with('The `kdk` is no longer available after `kdk update`. This is unexpected, please report this in https://github.com/khulnasoft/khulnasoft-development-kit/-/issues/2388.')
