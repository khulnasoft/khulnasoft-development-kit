  ...
  describe '#run' do
      ...
      context 'and without any errors' do
        ...
        context 'by setting KDK_CLEANUP_SOFTWARE to false', :hide_output do
          it 'does not prompt about uninstalling unnecessary software' do
            ...
            stub_prompt('n')
            expect(KDK::Output).not_to receive(:puts).with('- Uninstall any asdf software that is not defined in .tool-versions', stderr: true)
            expect(subject.run).to be_truthy
          end
    ...
    def expect_warn_and_puts(show_uninstall_software: true)
      ...
      if show_uninstall_software
        expect(KDK::Output).to receive(:puts).with('- Uninstall any asdf software that is not defined in .tool-versions:', stderr: true).ordered
