...
if defined?(TTY::Markdown)
  module KDKMarkdown
    # Overwrite convert_codespan to use blue color instead of white,
    # so inline code snippets look more prominent.
    def convert_codespan(el, opts)
      if @current_indent.zero? && !el.options[:lang] && el.value.lines.count == 1
        @pastel.blue(el.value)
      else
        super
      end
    end
