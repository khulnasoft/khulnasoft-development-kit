...
module Runit
  class Config
    ...
    # Create runit `run` and `finish` executable
    def create_runit_service(service)
      files = %w[run finish]

      files.each do |file|
        template_path = "runit/#{file}.sh.erb"
        template_content = render_template(
          template_path,
          service_instance: service,
          redacted_command: KDK::ConfigRedactor.new.redact_logfile(service.command),
          sv_dir: sv_dir(service)
        )
        file_path = sv_dir(service).join(file)
        write_executable_file(file_path, template_content)
      end
    end
