    ...
    context 'on darwin platforms' do
      ...
      context 'when homebrew is available' do
        ...
        it 'sets successfully sets all required environment variables' do
          ...
          expect(ENV.fetch('LDFLAGS', nil)).to eq("-L#{openssl_prefix}/lib -L#{icu4c_prefix}/lib")
          expect(ENV.fetch('CPPFLAGS', nil)).to eq("-I#{openssl_prefix}/include -I#{icu4c_prefix}/include")
