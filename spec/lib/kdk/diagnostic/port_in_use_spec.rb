  ...
  describe '#success?' do
    context 'when no process is using the port' do
      before do
        ...
        allow_kdk_shellout_command("lsof -ti :#{port} -sTCP:LISTEN", chdir: KDK.root).and_return(lsof_double)
      end
    ...
    context 'when port is in use by a KDK process' do
      before do
        ...
        cwd_double = kdk_shellout_double(run: '/path/to/kdk')

        allow_kdk_shellout_command("lsof -ti :#{port} -sTCP:LISTEN", chdir: KDK.root).and_return(lsof_double)
        allow_kdk_shellout_command('ps -p 1234 -o args=', chdir: KDK.root).and_return(ps_double)
        allow_kdk_shellout_command("lsof -a -d cwd -p 1234", chdir: KDK.root).and_return(cwd_double)
      end
    ...
    context 'when port is in use by an external process' do
      before do
        ...
        cwd_double = kdk_shellout_double(run: '/other')
