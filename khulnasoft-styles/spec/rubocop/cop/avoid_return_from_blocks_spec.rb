...
require_relative '../../../lib/rubocop/cop/avoid_return_from_blocks'
...
RSpec.describe Rubocop::Cop::AvoidReturnFromBlocks do
  ...
  shared_examples 'examples with allowed method' do |allowed_method|
    it "doesn't flag violation for return inside #{allowed_method}" do
      expect_no_offenses(<<~RUBY)
        items.#{allowed_method} do |item|
          do_something
          return if something_else
        end
      RUBY
  ...
  %i[each each_filename times loop].each do |allowed_method|
    it_behaves_like 'examples with allowed method', allowed_method
  end
