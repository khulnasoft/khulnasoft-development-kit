...
module KDK
  class Config < ConfigSettings
    ...
    def kdk_root
      Pathname.new(self.class::KDK_ROOT)
    end
    ...
    settings :repositories do
      ...
      string(:khulnasoft) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft.git' }
      ...
      string(:khulnasoft_docs) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft-docs.git' }
      string(:khulnasoft_docs_hugo) { 'https://khulnasoft.com/khulnasoft-org/technical-writing-group/khulnasoft-docs-hugo.git' }
      string(:khulnasoft_elasticsearch_indexer) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft-elasticsearch-indexer.git' }
      ...
      string(:khulnasoft_pages) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft-pages.git' }
      string(:khulnasoft_shell) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft-shell.git' }
      string(:khulnasoft_topology_service) { 'https://khulnasoft.com/khulnasoft-org/cells/topology-service.git' }
      string(:khulnasoft_runner) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft-runner.git' }
      string(:khulnasoft_ui) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft-ui.git' }
      string(:khulnasoft_zoekt_indexer) { 'https://khulnasoft.com/khulnasoft-org/khulnasoft-zoekt-indexer.git' }
      string(:omnibus_khulnasoft) { 'https://khulnasoft.com/khulnasoft-org/omnibus-khulnasoft.git' }
      string(:openbao) { 'https://github.com/openbao/openbao.git' }
      ...
      string(:duo_workflow_service) { 'https://khulnasoft.com/khulnasoft-org/duo-workflow/duo-workflow-service.git' }
    ...
    settings :kdk do
      ...
      settings :experimental do
        bool(:auto_reconfigure) { config.kdk.auto_reconfigure? }
      end
    ...
    settings :khulnasoft_docs do
      bool(:enabled) { false }
      bool(:auto_update) { true }
      bool(:https) { false }
      port(:port, 'khulnasoft_docs')
      port(:port_https, 'khulnasoft_docs_https')

      anything :__uri do
        klass = config.khulnasoft_docs.https? ? URI::HTTPS : URI::HTTP
        port = config.khulnasoft_docs.https? ? config.khulnasoft_docs.port_https : config.khulnasoft_docs.port
        klass.build(host: config.hostname, port: port)
      end

      string(:__listen_address) { "#{config.nginx.listen_address}:#{config.khulnasoft_docs.port}" }
      string(:__https_listen_address) { "#{config.nginx.listen_address}:#{config.khulnasoft_docs.port_https}" }

      bool(:__all_configured?) { config.khulnasoft_docs.enabled? && config.khulnasoft_runner.enabled? && config.omnibus_khulnasoft.enabled? && config.charts_khulnasoft.enabled? && config.khulnasoft_operator.enabled? }

      string(:__nanoc_cmd_common) { "--host #{config.hostname} --port #{config.khulnasoft_docs.port}" }
      string(:__nanoc_view_cmd) { "make compile && ../support/bundle-exec nanoc view #{config.khulnasoft_docs.__nanoc_cmd_common}" }
    end

    settings :khulnasoft_docs_hugo do
      bool(:enabled) { false }
      bool(:auto_update) { true }
      bool(:run_migration_scripts) { false }
      port(:port, 'khulnasoft_docs_hugo')
    end
    ...
    settings :cells do
      ...
      integer(:global_sequence_range) { CellManager::DEFAULT_SEQUENCE_RANGE }

      settings_array :instances, size: -> { config.cells.instance_count } do |i|
        integer(:id) { i + 1 + CellManager::LEGACY_CELL_ID }
        string(:name) { "cell-#{id}" }
        array(:sequence_range) do
          [
            1 + ((i + 1) * parent.parent.global_sequence_range),
            (i + 2) * parent.parent.global_sequence_range
          ]
        end
...

          {
            ...
            'khulnasoft' => {
              ...
              'rails' => {
                ...
                'session_store' => {
                  'session_cookie_token_prefix' => name
                }
              }
            }
          }
    ...
    settings :zoekt do
      bool(:enabled) { false }
      bool(:auto_update) { true }
      string(:version) { '404b1ff8c8119ac7bba8e5b5481937eb9dd02d50' }
      port(:web_port_test, 'zoekt-webserver-test')
      port(:web_port_dev_1, 'zoekt-webserver-development-1')
      port(:web_port_dev_2, 'zoekt-webserver-development-2')
    ...
    settings :openbao do
      bool(:enabled) { false }
      bool(:auto_update) { true }
      path(:bin) { find_executable!('bao') || '/usr/local/bin/bao' }
    ...
    settings :khulnasoft do
      ...
      settings :rails do
        ...
        settings :session_store do
          ...
          string(:session_cookie_token_prefix) do
            config.khulnasoft.topology_service.enabled? ? config.khulnasoft.cell.name : ""
          end
        end
      ...
      settings :cell do
        ...
        string(:name) { "cell-#{id}" }
        bool(:skip_sequence_alteration) { false }
      end
    end
