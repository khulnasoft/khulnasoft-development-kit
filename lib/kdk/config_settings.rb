...
module KDK
  class ConfigSettings
    ...
    class << self
      ...
      def hash_setting(key, stringify_values: false, merge: false, &blk)
        def_attribute(key, ConfigType::Hash, stringify_values: stringify_values, merge: merge, &blk)
      end
    ...
    def bury_hash!(hash = {})
      ...
      until stack.empty?
        path, current_hash = stack.pop

        current_hash.each do |key, value|
          current_path = path + [key]

          if value.is_a?(Hash)
            stack.push([current_path, value])
          else
            key_value_pairs[current_path] = value
          end
        end
      end

      bury_multiple!(key_value_pairs)
    end
