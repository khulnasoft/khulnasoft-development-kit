...
postgresql-replica/data:
	$(Q)support/tool-version-manager-exec . pg_basebackup -R -h ${postgresql_dir} -D ${postgresql_replica_data_dir} -P -U khulnasoft_replication --wal-method=fetch

postgresql-replica-2/data:
	$(Q)support/tool-version-manager-exec . pg_basebackup -R -h ${postgresql_dir} -D ${postgresql_replica_data_dir2} -P -U khulnasoft_replication --wal-method=fetch

postgresql-replication-primary-create-slot: postgresql-replication/slot

postgresql-replication/backup:
	$(Q)kdk rake geo:replication_backup
