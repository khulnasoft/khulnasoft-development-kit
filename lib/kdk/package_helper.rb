  ...
  class PackageHelper
    ...
    def create_package
      if File.exist?(package_path)
        KDK::Output.info("Skipping package creation because #{package_path} already exists.")
        return
      end
    ...
    def download_package(retries: 3, skip_cache: false, return_path: false)
      ...
      return KDK::Output.success("No changes detected in #{project_path}, skipping package download and extraction.") unless skip_cache || current_commit_sha != stored_commit_sha
      ...
      return package_path if return_path
    ...
    def sha_file_path
      File.join(sha_file_root, '.cache', ".#{package_basename}_commit_sha")
    end
