...
module KDK
  module Services
    class KhulnasoftHttpRouter < Base
      BASE_COMMAND = "support/exec-cd khulnasoft-http-router npm run dev -- -c wrangler.kdk.toml"
      ...
      def env
        {
          WRANGLER_LOG_PATH: config.kdk_root.join('tmp/log/khulnasoft-http-router.log')
        }.merge(https_env)
      end
