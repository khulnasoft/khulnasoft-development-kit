...
RSpec.describe KDK::Config do
  ...
  describe 'repositories' do
    describe 'khulnasoft_ui' do
      it 'returns the khulnasoft-ui repository URL' do
        expect(config.repositories.khulnasoft_ui).to eq('https://khulnasoft.com/khulnasoft-org/khulnasoft-ui.git')
      end
    end
  end
  ...
  describe 'khulnasoft' do
    ...
    describe 'rails' do
      ...
      describe 'cell' do
        describe '#cell_id' do
          it 'defaults to cell-1' do
            expect(config.khulnasoft.cell.name).to eq("cell-1")
  ...
  describe 'registry' do
    ...
    describe '#__listen' do
      it 'returns kdk.example.com:5000 by default' do
        expect(config.registry.__listen).to eq('kdk.example.com:5000')
      end
    end
  ...
  describe 'khulnasoft_docs' do
    describe 'enabled' do
      it 'is disabled by default' do
        expect(config.khulnasoft_docs.enabled).to be(false)
        expect(config.khulnasoft_docs.enabled?).to be(false)
        expect(config.khulnasoft_docs?).to be(false)
      end
    end

    describe 'auto_update' do
      it 'is enabled by default' do
        expect(config.khulnasoft_docs.auto_update).to be(true)
        expect(config.khulnasoft_docs.auto_update?).to be(true)
      end
    end
    ...
    describe '#port' do
      context 'when port is not specified' do
        it 'returns the default port' do
          expect(config.khulnasoft_docs.port).to eq(3005)
        end
      end
      ...
      context 'when port is specified' do
        let(:yaml) do
          {
            'khulnasoft_docs' => { 'port' => 5555 }
          }
        end

        it 'returns the configured port' do
          expect(config.khulnasoft_docs.port).to eq(5555)
        end
      end
    end
    ...
    describe '#https' do
      it 'is false by default' do
        expect(config.khulnasoft_docs.https).to be false
      end
    end
    ...
    describe '#port_https' do
      context 'when port_https is not specified' do
        it 'returns the default port' do
          expect(config.khulnasoft_docs.port_https).to eq(3030)
        end
      end
      ...
      context 'when port is specified' do
        let(:yaml) do
          {
            'khulnasoft_docs' => { 'port_https' => 3006 }
          }
        end

        it 'returns the configured port' do
          expect(config.khulnasoft_docs.port_https).to eq(3006)
        end
      end
    end
    ...
    describe '__all_configured' do
      context 'when all documentation projects enabled' do
        let(:yaml) do
          {
            'khulnasoft_docs' => { 'enabled' => true },
            'khulnasoft_runner' => { 'enabled' => true },
            'omnibus_khulnasoft' => { 'enabled' => true },
            'charts_khulnasoft' => { 'enabled' => true },
            'khulnasoft_operator' => { 'enabled' => true }
          }
        end

        it 'returns true' do
          expect(config.khulnasoft_docs.__all_configured?).to be(true)
        end
      end
      ...
      context 'when all documentation projects not enabled' do
        let(:yaml) do
          {
            'khulnasoft_docs' => { 'enabled' => true },
            'khulnasoft_runner' => { 'enabled' => true },
            'omnibus_khulnasoft' => { 'enabled' => false },
            'charts_khulnasoft' => { 'enabled' => true },
            'khulnasoft_operator' => { 'enabled' => true }
          }
        end
        ...
        it 'returns false' do
          expect(config.khulnasoft_docs.__all_configured?).to be(false)
        end
      end

      describe '__nanoc_cmd_common' do
        context 'when KDK host and KhulnaSoft Docs port not configured' do
          let(:yaml) do
            {}
          end

          it 'nanoc is passed default options' do
            expect(config.khulnasoft_docs.__nanoc_cmd_common).to eq('--host 127.0.0.1 --port 3005')
          end
        end

        context 'when KDK host and KhulnaSoft Docs port are configured' do
          let(:yaml) do
            {
              'hostname' => 'kdk.test.com',
              'khulnasoft_docs' => { 'port' => 5555 }
            }
          end

          it 'nanoc is passed configured options' do
            expect(config.khulnasoft_docs.__nanoc_cmd_common).to eq('--host kdk.test.com --port 5555')
          end
        end
      end
    end
  ...
  describe 'khulnasoft_docs_hugo' do
    describe 'enabled' do
      it 'is disabled by default' do
        expect(config.khulnasoft_docs_hugo.enabled).to be(false)
        expect(config.khulnasoft_docs_hugo.enabled?).to be(false)
        expect(config.khulnasoft_docs_hugo?).to be(false)
      end
    end

    describe 'auto_update' do
      it 'is enabled by default' do
        expect(config.khulnasoft_docs_hugo.auto_update).to be(true)
        expect(config.khulnasoft_docs_hugo.auto_update?).to be(true)
      end
    end

    describe 'run_migration_scripts' do
      it 'is disabled by default' do
        expect(config.khulnasoft_docs_hugo.run_migration_scripts).to be(false)
        expect(config.khulnasoft_docs_hugo.run_migration_scripts?).to be(false)
      end
    end
    ...
    describe '#port' do
      context 'when port is not specified' do
        it 'returns the default port' do
          expect(config.khulnasoft_docs_hugo.port).to eq(1313)
        end
      end
      ...
      context 'when port is specified' do
        let(:yaml) do
          {
            'khulnasoft_docs_hugo' => { 'port' => 5555 }
          }
        end

        it 'returns the configured port' do
          expect(config.khulnasoft_docs_hugo.port).to eq(5555)
        end
      end
    end
  end
  ...
  describe 'openbao' do
    ...
    describe '#bin' do
      it 'defaults bin to /usr/local/bin/openbao when no executable can be found' do
        stub_env('PATH', tmp_path)

        expect(default_config.openbao.bin).to eq(Pathname.new('/usr/local/bin/bao'))
      end

      it 'returns bin full path based on find_executable' do
        unstub_find_executable
        stub_env('PATH', tmp_path)
        custom_bin_path = Pathname.new(create_dummy_executable('bao'))

        expect(default_config.openbao.bin).to eq(custom_bin_path)
      end
    end
