...
mise_enabled() {
  ...
  if [[ "${TOOL_VERSION_MANAGER:-asdf}" == "mise" ]]; then
...
prefix_with_asdf_if_available() {
  ...
  elif mise_command_enabled "$1"; then
...
kdk_install_gem() {
  (
    ...
    if ! echo_if_unsuccessful prefix_with_asdf_if_available gem build khulnasoft-development-kit.gemspec; then
      return 1
    fi

    if ! echo_if_unsuccessful prefix_with_asdf_if_available gem install khulnasoft-development-kit-*.gem; then
      return 1
    fi
...
common_preflight_checks() {
  ...
  if ! asdf_check_rvm_rbenv; then
    ...
    echo "INFO: For more information, see https://github.com/khulnasoft/khulnasoft-development-kit/-/blob/main/doc/migrate_to_asdf.md." >&2
    return 1
  fi
}
