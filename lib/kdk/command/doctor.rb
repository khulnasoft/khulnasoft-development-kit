...
module KDK
  module Command
    class Doctor < BaseCommand
      help 'Run diagnostics on KDK'

      def self.track_telemetry?(_ = [])
        !(KDK::Telemetry.team_member? && ENV.fetch('ANTHROPIC_API_KEY', nil))
      end
      ...
      def run(args = [])
        ...
        @postgres_available = start_necessary_services
        ...
        if use_ai_assistance?
          out.info('KhulnaSoft is looking for a possible solution ...')
          begin
            DuoConnector.new(out).call(
              'Explain how to resolve the following diagnostics found by my local KDK:',
              uncorrectable_results.map(&:message)
            )
          rescue DuoConnector::DuoConnectorError => e
            out.error(e)
          end
        end

        # We don't need a message, we've already printed info.
        raise UserInteractionRequired, "" unless diagnostic_results.empty?
      ...
      def diagnostics
        unless @postgres_available
          return @diagnostics.reject do |diag|
            diag.instance_of?(Diagnostic::Postgresql) || diag.instance_of?(Diagnostic::PendingMigrations) || diag.instance_of?(Diagnostic::Praefect)
          end
        end
      ...
      def uncorrectable_results
        @uncorrectable_results ||= diagnostic_results.reject(&:correctable?)
      end
      ...
      def use_ai_assistance?
        KDK::Telemetry.team_member? && uncorrectable_results.any?
      end
      ...
      def start_necessary_services
        ...
        postgresql.ready?(try_times: 20, interval: 0.5).tap do |ready|
          out.warn('Database could not be started! We will run limited scope of diagnostics.') unless ready
        end
      end
