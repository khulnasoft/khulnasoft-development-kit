...
RSpec.describe KDK::ConfigSettings do
  ...
  describe '#bury!' do
    ...
    it 'buries with key as multiple arguments' do
      described_class.settings(:foo) { string(:name) { 'bonza' } }
      expect { config.bury!('foo', 'name', 'ripper') }
        .to change { config.yaml }.to('foo' => { 'name' => 'ripper' })
    end
  ...
  describe '#bury_multiple!' do
    it 'calls bury! for each key-value pair' do
      allow(config).to receive(:bury!)

      config.bury_multiple!({ 'port' => 8000, %w[foo name] => 'ripper' })

      expect(config).to have_received(:bury!).with('port', 8000)
      expect(config).to have_received(:bury!).with('foo', 'name', 'ripper')
    end
  end

  describe '#bury_hash!' do
    before do
      allow(config).to receive(:bury_multiple!)
    end

    it 'has an empty hash as default argument' do
      expect { config.bury_hash! }.not_to raise_error
      expect(config).to have_received(:bury_multiple!).with({})
    end

    it 'calls bury_multiple! with key-value pairs' do
      config.bury_hash!({ 'foo' => { 'name' => 'ripper' } })
      expect(config).to have_received(:bury_multiple!).with({ %w[foo name] => 'ripper' })
    end
  end
