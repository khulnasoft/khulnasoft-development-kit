  ...
  module Command
    ...
    class Pristine < BaseCommand
      ...
      def kdk_tmp_clean
        ...
        shellout(GIT_CLEAN_TMP_CMD, chdir: config.kdk_root)
      end
      ...
      def reset_configs
        shellout(RESET_CONFIGS_CMD, chdir: config.kdk_root)
      end

      def kdk_bundle_install
        shellout(bundle_install_cmd, chdir: config.kdk_root)
      end

      def kdk_bundle_pristine
        shellout(BUNDLE_PRISTINE_CMD, chdir: config.kdk_root)
      end
      ...
      def shellout(cmd, chdir: config.kdk_root)
