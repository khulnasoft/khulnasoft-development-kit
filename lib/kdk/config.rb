...
module KDK
  class Config < ConfigSettings
    ...
    hash_setting(:env, stringify_values: true, merge: true) do
      {
        ...
        # AI Gateway development environment variables
        case config.khulnasoft_ai_gateway.environment.to_s
        when 'local'
          env['DEVELOPMENT_AI_GATEWAY_URL'] = config.khulnasoft_ai_gateway.__listen.to_s
          env.delete('CLOUD_CONNECTOR_BASE_URL')
        when 'staging'
          env['CLOUD_CONNECTOR_BASE_URL'] = 'https://cloud.staging.khulnasoft.com'
          env.delete('DEVELOPMENT_AI_GATEWAY_URL')
        end
      end
    end
    ...
    settings :ai_services do
      bool(:enabled) { false }
    end

    settings :khulnasoft_ai_gateway do
      bool(:enabled) { config.ai_services.enabled? }
    ...
    settings :khulnasoft_http_router do
      ...
      string(:khulnasoft_rules_config) { 'session_token' }
    ...
    settings :khulnasoft_topology_service do
      ...
      port(:metrics_port, 'khulnasoft_topology_service_metrics')
      ...
      string(:database_name) { 'topology_service' }
    end
    ...
    settings :repositories do
      ...
      string(:khulnasoft_elasticsearch_indexer) { 'https://github.com/khulnasoft/khulnasoft-elasticsearch-indexer.git' }
      string(:khulnasoft_k8s_agent) { 'https://github.com/khulnasoft/cluster-integration/khulnasoft-agent.git' }
      string(:khulnasoft_operator) { 'https://github.com/khulnasoft/cloud-native/khulnasoft-operator.git' }
      string(:khulnasoft_pages) { 'https://github.com/khulnasoft/khulnasoft-pages.git' }
      string(:khulnasoft_shell) { 'https://github.com/khulnasoft/khulnasoft-shell.git' }
      string(:khulnasoft_topology_service) { 'https://github.com/khulnasoft/cells/topology-service.git' }
      string(:khulnasoft_runner) { 'https://github.com/khulnasoft/khulnasoft-runner.git' }
      string(:khulnasoft_zoekt_indexer) { 'https://github.com/khulnasoft/khulnasoft-zoekt-indexer.git' }
      string(:omnibus_khulnasoft) { 'https://github.com/khulnasoft/omnibus-khulnasoft.git' }
      string(:openbao_internal) { 'https://github.com/khulnasoft/govern/secrets-management/openbao-internal.git' }
      string(:registry) { 'https://github.com/khulnasoft/container-registry.git' }
      string(:khulnasoft_observability_backend) { 'https://github.com/khulnasoft/opstrace/opstrace.git' }
      string(:siphon) { 'https://github.com/khulnasoft/analytics-section/siphon.git' }
      string(:duo_workflow_executor) { 'https://github.com/khulnasoft/duo-workflow/duo-workflow-executor.git' }
    end
    ...
    settings :kdk do
      ...
      bool(:use_precompiled_ruby) { true }
      ...
      bool(:shell_completion) { false }
      array(:protected_config_files) { [] }
      settings :license_provisioning do
        bool(:enabled) { false }
        string(:edition) { 'self_managed' }
        settings :khulnasoft do
          # we can add seats later
          string(:tier) { 'ultimate' }
        end
        settings :duo do
          # we can add seats later
          string(:tier) { '' }
    ...
    settings :registry do
      ...
      string(:version) { 'v4.31.0-khulnasoft' }
    end
      ...
      hash_setting(:objects) do
        {
          ...
          'gitaly_bundles' => { 'bucket' => 'gitaly-bundles' }
        }
      end
    end
    ...
    settings :khulnasoft_k8s_agent do
      ...
      string(:__url_for_agentk) do
        if config.nginx?
          ...
          "ws://#{config.khulnasoft_k8s_agent.agent_listen_address}#{config.khulnasoft_k8s_agent.__agent_listen_url_path}"
        else
          raise UnsupportedConfiguration, "Unsupported listen network #{config.khulnasoft_k8s_agent.agent_listen_network}"
        end
      end
      string(:__grpc_url_for_agentk) do
        if config.nginx? && config.https? && config.nginx.http2?
          "grpcs://#{config.nginx.__listen_address}"
        else
    ...
    settings :workspaces do
      string(:api_host_url_prefix) { "/workspaces" }
      bool(:enabled) { false }

      settings :listen do
        string(:address) { "#{config.listen_address}:8160" }
        string(:listen_grace_period) { "5s" }
        string(:network) { "tcp" }
        string(:shutdown_grace_period) { "3600s" }
      end

      port(:port, "workspaces_nginx")

      settings :tls do
        string(:certificate) { config.kdk_root.join("_wildcard.workspaces.#{config.hostname}.pem") }
        string(:key) { config.kdk_root.join("_wildcard.workspaces.#{config.hostname}-key.pem") }
      end

      string(:hostname) { "workspaces.#{config.hostname}" }
    end
    ...
    settings :cells do
      ...
      settings_array :instances, size: -> { config.cells.instance_count } do |i|
        ...
        array(:__sequence_range) do
          [
            (i + 1) * parent.parent.__global_sequence_range,
            ((i + 2) * parent.parent.__global_sequence_range) - 1
          ]
        end
        hash_setting(:config, merge: true) do
          main_config = parent.config
          topology_service_client = main_config.khulnasoft.cell.topology_service_client
            ...
            'khulnasoft' => {
              'cell' => {
                ...
                'topology_service_client' => {
                  'enabled' => topology_service_client.enabled,
                  'address' => topology_service_client.address,
                  'ca_file' => topology_service_client.ca_file.to_s,
                  'certificate_file' => topology_service_client.certificate_file.to_s,
                  'private_key_file' => topology_service_client.private_key_file.to_s,
                  'tls' => {
                    'enabled' => topology_service_client.tls.enabled
                  }
                }
              },
              'rails' => {
    ...
    settings :gitaly do
      ...
      settings :bundle_uri do
        bool(:enabled) { config.object_store? }
        string(:go_cloud_url) do
          bucket = config.object_store.objects['gitaly_bundles']['bucket']
          connection = config.object_store.connection
          case connection['provider']
          when 'AWS'
            disable_ssl = connection['endpoint'].to_s.start_with?('http://')
    ...
    settings :openbao do
      ...
      string(:database_name) { "khulnasofthq_development_openbao" }
      bool(:metrics_enabled) { false }
      bool(:admin_enabled) { false }
      path(:log_dir) { config.kdk_root.join('log', 'openbao') }
    end
    ...
    settings :smartcard do
      bool(:enabled) { false }
      string(:hostname) { "smartcard.#{config.hostname}" }
      port(:port, 'smartcard_nginx')
      bool(:san_extensions) { true }
      settings :ssl do
        string(:certificate) { "smartcard.#{config.hostname}.pem" }
        string(:key) { "smartcard.#{config.hostname}-key.pem" }
    ...
    settings :khulnasoft do
      ...
      integer(:gitaly_client_max_attempts) { 4 }
      string(:gitaly_client_max_backoff) { '1.4s' }
      settings :rails do
        ...
        settings :session_store do
          ...
          bool(:unique_cookie_key_postfix) do
            topology_service_client = config.khulnasoft.cell.topology_service_client
            !topology_service_client.enabled? && !config.khulnasoft_topology_service.enabled
          end
          string(:cookie_key) { "_khulnasoft_session" }
          string(:session_cookie_token_prefix) do
            topology_service_client = config.khulnasoft.cell.topology_service_client
            topology_service_client.enabled? ? "cell-#{config.khulnasoft.cell.id}" : ''
          end
        end

        array(:allowed_hosts) { [] }
        array(:action_cable_allowed_origins) { [] }
      ...
      settings :cell do
        ...
        settings :topology_service_client do
          ...
          hash_setting(:metadata) do
            {
              'x-client-runway-cc-subject-dn' => ConfigHelper.generate_dn(config.nginx? ? config.nginx.__listen_address : config.workhorse.__listen_address, 'cells')
            }
          end
        end
      end
    ...
    settings :tool_version_manager do
      # If mise is not enabled, the user manages their own tool versions.
      bool(:enabled) { config.mise.enabled? }
    end
