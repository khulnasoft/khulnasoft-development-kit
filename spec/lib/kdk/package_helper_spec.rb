...
RSpec.describe KDK::PackageHelper do
  ...
  describe '#initialize' do
    ...
    context 'when project_id is provided' do
      ...
      it 'sets instance variables correctly' do
        ...
        expect(package_helper.download_path).to eq(Pathname.new('/path/to/download'))
  ...
  describe '#upload_package' do
    let(:platform_specific) { false }
    let(:uri) { URI.parse('https://khulnasoft.com/api/v4/projects/74823/packages/generic/test_package/1.0.0/test_package.tar.gz') }
    ...
    shared_examples 'uploads the package successfully' do
      it 'uploads the package' do
        expect(package_helper).to receive(:create_package)
        expect(request).to receive(:[]=).with('JOB-TOKEN', 'test_token')
        expect(request).to receive(:body=).with('package content')
        expect(response).to receive(:is_a?).with(Net::HTTPSuccess).and_return(true)
        expect(KDK::Output).to receive(:success).with(include('Package uploaded successfully'))
    ...
    it 'raises an error when upload fails' do
      ...
      expect(package_helper).to receive(:create_package)
      expect { package_helper.upload_package }.to raise_error(RuntimeError, /Upload failed: Error message/)
    end

    context 'when platform_specific is true' do
      let(:platform_specific) { true }
      let(:uri) { URI.parse('https://khulnasoft.com/api/v4/projects/74823/packages/generic/test_package-linux-amd64/1.0.0/test_package.tar.gz') }

      context 'and OS and architecture are supported' do
        include_examples 'uploads the package successfully'
      end

      context 'when BUILD_ARCH is set to arm64' do
        let(:stubbed_env) { { 'BUILD_ARCH' => 'arm64' } }
        let(:uri) { URI.parse('https://khulnasoft.com/api/v4/projects/74823/packages/generic/test_package-linux-arm64/1.0.0/test_package.tar.gz') }
  ...
  describe '#download_package' do
    let(:uri) { URI.parse("https://khulnasoft.com/api/v4/projects/74823/packages/generic/#{expected_package_name}/1.0.0/test_package.tar.gz") }
    ...
    before do
      package_config[:download_path] = tmpdir
    ...
    context 'when platform_specific is true' do
      ...
      context 'and OS and architecture are not supported' do
        ...
        it 'does not download the package' do
          ...
          expect(Net::HTTP).not_to receive(:get_response).with(uri)
    ...
    context 'when download fails' do
      ...
      before do
        stub_request(:get, uri.to_s).to_return(status: 502, body: 'Error message')
      end
