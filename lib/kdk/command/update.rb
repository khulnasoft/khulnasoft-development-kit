...
module KDK
  module Command
    ...
    class Update < BaseCommand
      help 'Update all services and install dependencies'

      KdkNotFoundError = Class.new(StandardError)
      ...
      def run(_args = [])
        ...
        success = run_rake(:reconfigure) if success

        duration = Process.clock_gettime(Process::CLOCK_MONOTONIC) - started_at
        formatted_duration = Utils.format_duration(duration)

        if success
          Announcements.new.render_all
          KDK::Output.success("Successfully updated in #{formatted_duration}!")
          run_weekly_diagnostics
        else
          KDK::Output.error("Failed to update in #{formatted_duration}.", report_error: false)
      ...
      def setup_sandbox
        return unless config.kdk.sandbox.managed
      ...
      def check_kdk_available
        ...
        out.error('The `kdk` is no longer available after `kdk update`. This is unexpected')

        KDK::Telemetry.capture_exception(KdkNotFoundError.new('`kdk` command is no longer available'))
      end
