  ...
  describe '#run' do
    ...
    context 'when all commands succeed' do
      it 'displays an informational message and returns true', :hide_stdout do
        ...
        # kdk_tmp_clean
        expect_kdk_shellout_command(described_class::GIT_CLEAN_TMP_CMD, chdir: config.kdk_root).and_return(shellout_double)

        # kdk_bundle
        expect_kdk_shellout_command(subject.bundle_install_cmd, chdir: config.kdk_root).and_return(shellout_double)
        expect_kdk_shellout_command(described_class::BUNDLE_PRISTINE_CMD, chdir: config.kdk_root).and_return(shellout_double)

        # reset_configs
        expect_kdk_shellout_command(described_class::RESET_CONFIGS_CMD, chdir: config.kdk_root).and_return(shellout_double)
