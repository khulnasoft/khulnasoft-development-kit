...
class SetupWorkspace
  ...
  def execute_bootstrap
    ...
    success = KDK::Shellout.new('support/khulnasoft-remote-development/remote-development-kdk-bootstrap.sh', chdir: ROOT_DIR).execute
  ...
  def configure_telemetry
    username = KDK::Output.prompt(KDK::Telemetry::PROMPT_TEXT)
    KDK::Telemetry.update_settings(username)
  end

  def send_telemetry(success, duration)
    KDK::Telemetry.send_telemetry(success, 'setup-workspace', { duration: duration, environment: 'remote-development', platform: KDK::Machine.platform, architecture: KDK::Machine.architecture })
