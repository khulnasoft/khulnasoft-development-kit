...
RSpec.describe KDK::Command::Report, :kdk_root do
  ...
  let(:os_name) { 'Darwin 24.5.0' }
  let(:arch) { 'arm64' }
  let(:ruby_version) { 'ruby 3.3.8' }
  let(:kdk_version) { 'd0gf00d' }
  ...
  describe '#run' do
    before do
      ...
      allow_shellout_run('uname -moprsv', os_name)
      allow_shellout_run('arch', arch)
      allow_shellout_run('ruby --version', ruby_version)
      allow_shellout_run('git rev-parse --short HEAD', kdk_version)
    end
  ...
  def allow_shellout_run(command, result)
    sh = kdk_shellout_double(run: result)
    allow_kdk_shellout_command(command).and_return(sh)
  end
end
