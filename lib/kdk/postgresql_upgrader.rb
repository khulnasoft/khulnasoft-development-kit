...
module KDK
  class PostgresqlUpgrader
    ...
    def available_versions
      @available_versions ||=
        if KDK::Dependencies.tool_version_manager_available?
          ...
          raise 'Only mise, brew and apt based Linux systems supported.'
        end
    end
    ...
    def mise_available_versions
      ...
      JSON.parse(lines)
        .map { |entry| entry.merge('gem_version' => Gem::Version.new(entry['version'])) }
        .group_by { |x| x['gem_version'].canonical_segments.first }
        .transform_values { |group| group.max_by { |x| x['gem_version'] } }
        .transform_values { |entry| "#{entry['install_path']}/bin" }
    end
