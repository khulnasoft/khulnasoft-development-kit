...
RSpec.describe KDK::Diagnostic::PendingMigrations do
  ...
  def stub_pending_migrations(success)
    ...
    allow_kdk_shellout_command(get_psql_command('khulnasofthq_development'), chdir: KDK.root).and_return(sh)
    allow_kdk_shellout_command(get_psql_command('khulnasofthq_development_ci'), chdir: KDK.root).and_return(sh)
