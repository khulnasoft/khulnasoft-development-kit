...
ifeq ($(asdf_opt_out),false)
	@echo
	@echo "${DIVIDER}"
	@echo "Installing asdf tools from ${registry_dir}/.tool-versions (or default to Go 1.22)"
	@echo "${DIVIDER}"
	$(Q)cd ${registry_dir} && if [ ! -f .tool-versions ]; then echo "golang 1.22.0" > .tool-versions; fi
	$(Q)cd ${registry_dir} && ASDF_DEFAULT_TOOL_VERSIONS_FILENAME="${registry_dir}/.tool-versions" asdf install
else
	@true
endif
