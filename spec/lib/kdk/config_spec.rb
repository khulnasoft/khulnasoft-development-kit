...
RSpec.describe KDK::Config do
  ...
  describe '#postgresql' do
    ...
    describe '#active_version' do
      it { expect(default_config.postgresql.active_version).to eq('16.11') }
  ...
  describe 'khulnasoft' do
    ...
    describe '#gitaly_client_max_attempts' do
      it 'returns 4 by default' do
        expect(config.khulnasoft.gitaly_client_max_attempts).to be(4)
      end
    end

    describe '#gitaly_client_max_backoff' do
      it 'returns 1.4s by default' do
        expect(config.khulnasoft.gitaly_client_max_backoff).to be('1.4s')
      end
    end
    ...
    describe 'rails' do
      ...
      describe '#action_cable_allowed_origins' do
        it 'returns [] by default' do
          expect(config.khulnasoft.rails.action_cable_allowed_origins).to eq([])
        end

        context 'when array is specified' do
          let(:yaml) do
            {
              'khulnasoft' =>
                { 'rails' =>
                    { 'action_cable_allowed_origins' => ['http://localhost:5101', 'http://localost:3000'] } }
            }
          end

          it 'returns array from configuration file' do
            expect(config.khulnasoft.rails.action_cable_allowed_origins).to contain_exactly('http://localhost:5101', 'http://localost:3000')
          end
        end
      end
    ...
    describe 'cell' do
      ...
      describe 'topology_service_client' do
        describe '#enabled' do
          it 'defaults to true' do
            expect(config.khulnasoft.cell.topology_service_client.enabled).to be(true)
          end

          context 'replacing the default value' do
            before do
              yaml['khulnasoft'] = {
                'cell' => {
            ...
            it { expect(config.khulnasoft.cell.topology_service_client.enabled).to be(false) }
          end
        end

        describe '#address' do
          it 'defaults to the localhost:grpc_port' do
            expect(config.khulnasoft.cell.topology_service_client.address).to eq("kdk.example.com:9095")
          end
          ...
          context 'replacing the default value' do
            ...
            it { expect(config.khulnasoft.cell.topology_service_client.address).to eq('127.0.0.1:6000') }
          end
        end
        ...
        describe 'certificate files' do
          ...
          it 'they default to the certificate files in the temp directory' do
            expect(config.khulnasoft.cell.topology_service_client.ca_file).to eq(Pathname.new(File.join(certs_directory, "ca-cert.pem")))
            expect(config.khulnasoft.cell.topology_service_client.certificate_file).to eq(Pathname.new(File.join(certs_directory, "client-cert.pem")))
            expect(config.khulnasoft.cell.topology_service_client.private_key_file).to eq(Pathname.new(File.join(certs_directory, "client-key.pem")))
          end
          ...
          context 'replacing the default value' do
            before do
              yaml['khulnasoft'] = {
                'cell' => {
                  'topology_service_client' => {
                    'ca_file' => 'custom-ca-cert.pem',
                    'private_key_file' => 'custom-client-key.pem',
                    'certificate_file' => 'custom-client-cert.pem'
                  }
                }
              }
            end

            it 'reflects the changes in the config' do
              expect(config.khulnasoft.cell.topology_service_client.ca_file).to eq(Pathname.new('custom-ca-cert.pem'))
              expect(config.khulnasoft.cell.topology_service_client.private_key_file).to eq(Pathname.new('custom-client-key.pem'))
              expect(config.khulnasoft.cell.topology_service_client.certificate_file).to eq(Pathname.new('custom-client-cert.pem'))
            end
          end
        end
      end
  ...
  describe 'k8s_agent' do
    ...
    describe '__grpc_url_for_agentk' do
      let(:nginx_enabled) { false }
      let(:https_enabled) { false }
      let(:http2_enabled) { false }
      let(:yaml) do
        {
          'nginx' => { 'enabled' => nginx_enabled, 'http2' => { 'enabled' => http2_enabled } },
          'https' => { 'enabled' => https_enabled }
        }
      end

      context 'when nginx is enabled with HTTPS and HTTP/2' do
        let(:nginx_enabled) { true }
        let(:https_enabled) { true }
        let(:http2_enabled) { true }

        it 'returns grpcs URL with nginx listen address' do
          expect(config.khulnasoft_k8s_agent.__grpc_url_for_agentk).to eq('grpcs://127.0.0.1:8080')
        end
      end

      context 'when nginx is enabled but HTTPS is disabled' do
        let(:nginx_enabled) { true }

        it { expect(config.khulnasoft_k8s_agent.__grpc_url_for_agentk).to eq('grpc://127.0.0.1:8150') }
      end
    ...
    describe '__url_for_agentk' do
      ...
      context 'when nginx is not enabled' do
        ...
        it 'is grpc://127.0.0.1:8150' do
          expect(config.khulnasoft_k8s_agent.__url_for_agentk).to eq('ws://127.0.0.1:8150/-/kubernetes-agent')
        end
      end
  ...
  describe 'smartcard' do
    ...
    it 'has a hostname by default' do
      expect(config.smartcard.hostname).to eq('smartcard.kdk.example.com')
    end
    ...
    describe 'ssl' do
      it 'provides default cert' do
        expect(config.smartcard.ssl.certificate).to eq('smartcard.kdk.example.com.pem')
      end

      it 'provides default key' do
        expect(config.smartcard.ssl.key).to eq('smartcard.kdk.example.com-key.pem')
      end
  ...
  describe 'webpack' do
    ...
    describe '#__dev_server_public' do
      ...
      context 'when public_address is set' do
        before do
          yaml['webpack'] = { 'public_address' => "wss://port-3808.example.io/ws" }
        end
    ...
    describe '#version' do
      context 'when no version is specified' do
        it 'returns the default version' do
          expect(config.registry.version).to eq('v4.31.0-khulnasoft')
        end
      end
    end
  ...
  describe 'tool_version_manager' do
    describe 'enabled' do
      context 'when mise is enabled and asdf is opted out' do
        ...
        it 'is true by default' do
          expect(config.tool_version_manager.enabled).to be(true)
          expect(config.tool_version_manager.enabled?).to be(true)
        end
      end

      context 'when mise is disabled and asdf is not opted out' do
        let(:yaml) do
          { 'mise' => { 'enabled' => false },
            'asdf' => { 'opt_out' => false } }
        end
        ...
        it 'is false by default' do
          expect(config.tool_version_manager.enabled).to be(false)
          expect(config.tool_version_manager.enabled?).to be(false)
        end
      end

      context 'when mise is disabled and asdf is opted out' do
        let(:yaml) do
          { 'mise' => { 'enabled' => false },
            'asdf' => { 'opt_out' => true } }
        end
  ...
  describe 'khulnasoft_http_router' do
    ...
    describe '#khulnasoft_rules_config' do
      it 'defaults to `session_token`' do
        expect(config.khulnasoft_http_router.khulnasoft_rules_config).to eq('session_token')
      end
    end
  ...
  describe 'khulnasoft_topology_service' do
    describe '#enabled' do
      ...
      context 'replacing the default value' do
        before do
          yaml['khulnasoft_topology_service'] = {
            'enabled' => false
          }
        end

        it { expect(config.khulnasoft_topology_service.enabled).to be(false) }
      end
    end

    describe '#auto_update' do
      it 'defaults to true' do
        expect(config.khulnasoft_topology_service.auto_update).to be(true)
      end
      ...
      context 'replacing the default value' do
        ...
        it { expect(config.khulnasoft_topology_service.auto_update).to be(false) }
      end
    end
    ...
    describe '#grpc_port' do
      ...
      context 'replacing the default value' do
        ...
        it { expect(config.khulnasoft_topology_service.grpc_port).to eq(6000) }
      end
    end
    ...
    describe '#rest_port' do
      ...
      context 'replacing the default value' do
        ...
        it { expect(config.khulnasoft_topology_service.rest_port).to eq(6000) }
      end
    end

    describe '#metrics_port' do
      it 'defaults to 9105' do
        expect(config.khulnasoft_topology_service.metrics_port).to eq(9105)
      end
      ...
      context 'replacing the default value' do
        ...
        it { expect(config.khulnasoft_topology_service.metrics_port).to eq(6000) }
      end
    end
    ...
    describe 'certificate files' do
      ...
      it 'they default to the certificate files in the temp directory' do
        expect(config.khulnasoft_topology_service.certificate_file).to eq(Pathname.new(File.join(certs_directory, "server-cert.pem")))
        expect(config.khulnasoft_topology_service.key_file).to eq(Pathname.new(File.join(certs_directory, "server-key.pem")))
        expect(config.khulnasoft_topology_service.client_certificate_file).to eq(Pathname.new(File.join(certs_directory, "client-cert.pem")))
      end
      ...
      context 'replacing the default value' do
        before do
          yaml['khulnasoft_topology_service'] = {
            ...
            'client_certificate_file' => 'custom-client-cert.pem'
          }
        end

        it 'reflects the changes in the config' do
          expect(config.khulnasoft_topology_service.certificate_file).to eq(Pathname.new('custom-cert.pem'))
          expect(config.khulnasoft_topology_service.key_file).to eq(Pathname.new('custom-key.pem'))
          expect(config.khulnasoft_topology_service.client_certificate_file).to eq(Pathname.new('custom-client-cert.pem'))
        end
      end
    end
  end
