...
RSpec.describe KDK::Command::Doctor, :hide_output do
  ...
  context 'when postgresql is not ready' do
    ...
    it 'warns if services cannot be started' do
      expect(Runit).to receive(:start).with('postgresql', quiet: true)
      expect_any_instance_of(KDK::Postgresql).to receive(:ready?)
  ...
  context 'with failing diagnostics' do
    ...
    it 'runs all diagnosis' do
      ...
      expect { subject.run }.to raise_error(KDK::UserInteractionRequired)
    end
  ...
  context "when passing '--correct' flag" do
    ...
    context 'with correctable diagnostics' do
      ...
      it 'runs all diagnosis' do
        ...
        expect { subject.run(args) }.to raise_error(KDK::UserInteractionRequired)
      end
    ...
    context 'with uncorrectable diagnostics' do
      ...
      context 'with ai_assistance for troubleshooting' do
        let(:khulnasoft_auth_token) { '<your-khulnasoft-auth-token>' }
        let(:duo_connector) { KDK::DuoConnector.new(KDK::Output) }

        before do
          allow(KDK::Telemetry).to receive(:team_member?).and_return(true)
          allow(KDK::DuoConnector).to receive(:new).with(KDK::Output).and_return(duo_connector)
        end

        it 'connects to claude to retrieve guidance for the user' do
          expect(KDK::Output).to receive(:info).with('KhulnaSoft Duo is looking for a possible solution ...')
          expect(duo_connector).to receive(:call)
            .with(
