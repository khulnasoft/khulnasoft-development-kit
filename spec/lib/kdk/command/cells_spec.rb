...
RSpec.describe KDK::Command::Cells do
  ...
  context 'run in cell sub-command' do
    let(:args) { %w[cell-1 config get cells.enabled] }
    let(:cell_exist) { true }
    let(:success) { true }
    let(:sh) { kdk_shellout_double(success?: success) }

    before do
      allow_any_instance_of(CellManager).to receive(:cell_exist?).with('cell-1').and_return(cell_exist)
    end

    it 'runs CellManager#run_in_cell' do
      expect_any_instance_of(CellManager).to receive(:run_in_cell).with('cell-1', args.drop(1)).and_return(sh)
