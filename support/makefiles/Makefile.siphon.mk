...
ifeq ($(asdf_opt_out),false)
	...
	$(Q)cd ${khulnasoft_siphon_dir} && ASDF_DEFAULT_TOOL_VERSIONS_FILENAME="${khulnasoft_siphon_dir}/.tool-versions" asdf install
else ifeq ($(mise_enabled),true)
	@echo
	@echo "${DIVIDER}"
	@echo "Installing mise tools from ${khulnasoft_siphon_dir}/.tool-versions"
	@echo "${DIVIDER}"
	$(Q)cd ${khulnasoft_siphon_dir} && mise install -y
else
	@true
endif
...
siphon-update-run: ensure-siphon-stopped siphon/.git/pull siphon-common-setup

.PHONY: ensure-siphon-stopped
ensure-siphon-stopped:
	@echo
	@echo "${DIVIDER}"
	@echo "Ensuring siphon is stopped"
	@echo "${DIVIDER}"
	$(Q)kdk stop siphon
