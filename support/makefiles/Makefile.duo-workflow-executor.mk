...
duo_workflow_executor_version = $(shell support/resolve-dependency-commitish "${khulnasoft_development_root}/khulnasoft/DUO_WORKFLOW_EXECUTOR_VERSION")
...
ifeq ($(asdf_opt_out),false)
	...
	@echo "${DIVIDER}"
	$(Q)cd ${duo_workflow_executor_dir} && ASDF_DEFAULT_TOOL_VERSIONS_FILENAME="${duo_workflow_executor_dir}/.tool-versions" asdf install
else ifeq ($(mise_enabled),true)
	@echo
	@echo "${DIVIDER}"
	@echo "Installing mise tools from ${duo_workflow_executor_dir}/.tool-versions"
	@echo "${DIVIDER}"
	$(Q)cd ${duo_workflow_executor_dir} && mise install -y
else
	@true
endif
