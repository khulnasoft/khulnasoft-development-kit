...
module KDK
  module Dependencies
    class Checker
      ...
      EXPECTED_NODEJS_VERSION = '22.12.0'
      EXPECTED_POSTGRESQL_VERSION = KDK::Postgresql.target_version.to_s
      EXPECTED_REDIS_VERSION = KDK::Redis.target_version.to_s
      ...
      def initialize(preflight: false)
        @preflight = preflight
        @error_messages = []
      end

      def check_all
        # We need to refresh PATH to check binaries in newly added directories.
        KDK::Dependencies.refresh_env_path!
        ...
        check_ruby_gems_ok if @preflight
      end
      ...
      def check_mise_installed
        check_binary('mise') if config.tool_version_manager.enabled?
      end
      ...
      def check_ruby_version
        ...
        cmd = 'ruby --version'
        # .tool-versions may have changed during a `kdk update`, so we
        # should execute ruby using mise to avoid a stale PATH.
        cmd = "mise exec -- #{cmd}" if KDK::Dependencies.tool_version_manager_available?
        ...
        expected = Gem::Version.new(KhulnasoftVersions.new.ruby_version)
      ...
      def check_redis_version
        ...
        current_version = Checker.parse_version(`redis-server --version`, prefix: '(?:Redis|Valkey) server v=')
      ...
      def check_brew_dependencies_installed
        ...
        msg = <<~MESSAGE
          ...
          To install these dependencies, run the following command:
            ...
            (cd #{config.kdk_root} && brew bundle)
        MESSAGE
        @error_messages << msg
      end
      ...
      def check_exiftool_installed
        ...
        msg = "You may need to run 'brew reinstall exiftool'." if KDK::Machine.macos?
      ...
      def check_ruby_gems_ok
        checker = KDK::Diagnostic::RubyGems.new(allow_gem_not_installed: true)
