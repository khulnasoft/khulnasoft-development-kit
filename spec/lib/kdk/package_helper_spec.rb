...
RSpec.describe KDK::PackageHelper do
  ...
  describe '#download_package' do
    ...
    context 'when download fails' do
      ...
      it 'retries 3 times and raises an error' do
        expect(KDK::Output).to receive(:info).with(include('Downloading package from')).exactly(3).times
        expect { package_helper.download_package }.to raise_error(RuntimeError, /Download failed: Error message/)
      end

      it 'respects custom retry count' do
        expect(KDK::Output).to receive(:info).with(include('Downloading package from')).twice
        expect { package_helper.download_package(retries: 2) }.to raise_error(RuntimeError, /Download failed: Error message/)
      end
    end
