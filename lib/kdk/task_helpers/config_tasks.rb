  ...
  module TaskHelpers
    ...
    class ConfigTasks
      ...
      def self.define_template_tasks(tasks)
        ...
        KDK.components.each_value do |component|
          component.templates.each do |template_task|
            tasks.add_task(template_task)
          end
        end
      end
      ...
      def all_with_makefile_target
        all_tasks.select(&:generate_makefile_target).sort_by(&:name)
      end
    end
  end
end
