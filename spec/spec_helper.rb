...
def stub_tool_versions
  ...
  allow_any_instance_of(KDK::ToolVersionsUpdater).to receive(:tool_versions).and_return(tools)
end
...
def remove_memoized_instance_variable(object, name)
  object.remove_instance_variable(name) if object.instance_variable_defined?(name)
end
