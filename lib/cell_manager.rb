...
class CellManager
  ...
  PORT_OFFSET = 150
  DEFAULT_SEQUENCE_RANGE = 10**12
  LEGACY_CELL_ID = 1
  LEGACY_CELL_SEQUENCE_MAXVAL = DEFAULT_SEQUENCE_RANGE - 1
  ...
  def run_in_cell(cell_id, args, quiet: false, skip_directory_check: false)
    ...
    unless skip_directory_check || Dir.exist?(dir)
      KDK::Output.error("Cell #{cell[:id]} doesn’t exist yet, run `kdk cells up` first.")
      return Result.new(false)
    end
  ...
  def kdk_build(cell)
    ...
    if Dir.exist?(cell_kdk_root)
      ...
      sh = KDK::Shellout.new(*%w[git remote get-url origin])
      sh.execute
      original_origin = sh.success? ? sh.read_stdout.chomp : 'https://github.com/khulnasoft/khulnasoft-development-kit.git'
