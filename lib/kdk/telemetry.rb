...
module KDK
  module Telemetry
    # Allow KhulnasoftSDK to batch up to 100 payloads before flushing.
    ANALYTICS_BATCH_SIZE = 100

    ANALYTICS_APP_ID = 'e2e967c0-785f-40ae-9b45-5a05f729a27f'
    ANALYTICS_BASE_URL = 'https://kdk-telemetry.runway.khulnasoft.net'

    # Track events emitted on CI in https://github.com/khulnasoft/quality/tooling/kdk-playground/
    CI_ANALYTICS_APP_ID = '6a31192c-6567-40a3-9413-923abc790f05'
    CI_ANALYTICS_BASE_URL = ANALYTICS_BASE_URL
    ...
    def self.version_manager
      return 'mise' if KDK.config.tool_version_manager.enabled?
    ...
    def self.client
      # Memoize client per process to avoid broken HTTP state during forks.
      @client = nil if @pid != Process.pid
      ...
      @client = KhulnasoftSDK::Client.new(app_id: app_id, host: host, buffer_size: ANALYTICS_BATCH_SIZE)
    end

    def self.register_at_exit
      return if @register_at_exit
    ...
    def self.telemetry_host
      default = ENV['CI'] ? CI_ANALYTICS_BASE_URL : ANALYTICS_BASE_URL

      ENV.fetch('KHULNASOFT_SDK_HOST', default)
    end
    ...
    def self.init_sentry
      ...
      Sentry.init do |config|
        ...
        config.before_send = lambda do |event, hint|
          ...
          # See https://github.com/khulnasoft/opstrace/opstrace/-/issues/2842#note_1927103517
    ...
    def self.payload
      {
        ...
        platform_version: KDK::Machine.platform_version,
        ...
        sandbox: {
          enabled: SandboxManager.new.enabled?,
          managed: KDK.config.kdk.sandbox.managed
        },
        precompiled: {
          ruby: Utils.precompiled_ruby?
        },
        release: release
      }
    end
    ...
    def self.team_member?
      ...
      @team_member = git_emails.include?('@khulnasoft.com') ||
        git_emails(dir: KDK.config.khulnasoft.dir).include?('@khulnasoft.com')
    ...
    def self.send_custom_event(metric, value, extras: {})
      return unless telemetry_enabled?
      ...
      event_payload[:extras] = extras unless extras.empty?

      client.track("Custom #{metric}", event_payload)
      KDK::Output.debug("Custom telemetry event sent: metric=#{metric}, value=#{value}, extras=#{extras}")
    end

    def self.git_emails(dir: KDK.config.kdk_root)
      return "" unless Dir.exist?(dir)

      Shellout.new(%w[git config --get-all user.email], chdir: dir)
              .run
    end
    ...
    def self.release
      ...
      @release = begin
        sha, time = KDK::Shellout.new(%w[git show HEAD --format=%h;%cD --no-patch]).run.split(';')
        {
          sha: sha,
          date_unix: Time.parse(time).utc.to_i
        }
