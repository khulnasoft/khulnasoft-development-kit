...
RSpec.describe 'kdk' do
  ...
  shared_examples 'returns expected output' do
    it 'returns expected output' do
      expect(run("KDK_TELEMETRY=0 #{kdk_bin_full_path} #{command}")).to eql(expected_output)
    end
  end

  shared_examples 'contains expected output' do
    it 'contains expected output' do
      expect(run("KDK_TELEMETRY=0 #{kdk_bin_full_path} #{command}")).to include(expected_output)
    end
  end
