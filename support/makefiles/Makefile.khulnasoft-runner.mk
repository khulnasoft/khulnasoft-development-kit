...
ifeq ($(asdf_opt_out),false)
	...
	@echo "${DIVIDER}"
	$(Q)cd ${khulnasoft_runner_dir} && ASDF_DEFAULT_TOOL_VERSIONS_FILENAME="${khulnasoft_runner_dir}/.tool-versions" asdf install
else ifeq ($(mise_enabled),true)
	@echo
	@echo "${DIVIDER}"
	@echo "Installing mise tools from ${khulnasoft_runner_dir}/.tool-versions"
	@echo "${DIVIDER}"
	$(Q)cd ${khulnasoft_runner_dir} && mise install -y
else
	@true
endif
...
khulnasoft-runner/.git/pull: khulnasoft-runner/.git
	...
	@echo "${DIVIDER}"
	$(Q)support/component-git-update khulnasoft_runner "${khulnasoft_runner_clone_dir}" main main
