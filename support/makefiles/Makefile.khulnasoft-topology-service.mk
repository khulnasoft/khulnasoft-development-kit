...
ifeq ($(asdf_opt_out),false)
	...
	$(Q)cd ${khulnasoft_topology_service_dir} && ASDF_DEFAULT_TOOL_VERSIONS_FILENAME="${khulnasoft_topology_service_dir}/.tool-versions" asdf install
else ifeq ($(mise_enabled),true)
	@echo
	@echo "${DIVIDER}"
	@echo "Installing mise tools from ${khulnasoft_topology_service_dir}/.tool-versions"
	@echo "${DIVIDER}"
	$(Q)cd ${khulnasoft_topology_service_dir} && mise install -y
else
	@true
endif
...
khulnasoft-topology-service/.git/pull: khulnasoft-topology-service/.git
	...
	@echo "${DIVIDER}"
	$(Q)support/component-git-update khulnasoft_topology_service khulnasoft-topology-service main main
