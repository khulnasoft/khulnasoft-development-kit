...
1. Clone the [`khulnasoft-development-kit` project](https://github.com/khulnasoft/khulnasoft-development-kit) to `kdk`
   so you have access to the project's
   [`Brewfile`](https://github.com/khulnasoft/khulnasoft-development-kit/-/blob/main/Brewfile).
   You can:
     - Reuse an existing checkout if you have one, but make sure it's up to date.
     - Use this check out again when you [use KDK to install KhulnaSoft](index.md#use-kdk-to-install-khulnasoft).

1. Install [`nvm`](https://github.com/nvm-sh/nvm#installing-and-updating). It should automatically
   configure it for your shell.
1. Install [the KDK required version of node](https://github.com/khulnasoft/khulnasoft-development-kit/-/blob/main/.tool-versions) using `nvm`, such as:
...
1. Install [the KDK required version of Ruby](https://github.com/khulnasoft/khulnasoft-development-kit/-/blob/main/.ruby-version)
using rbenv, such as:
...
[KDK to manage their dependencies](index.md#install-dependencies).
...
1. Complete the rest of the [manual instructions](index.md#install-dependencies-manually), then
   [use KDK to install KhulnaSoft](index.md#use-kdk-to-install-khulnasoft).
...
[KDK to manage their dependencies with `asdf`](index.md#install-dependencies).
...
Now you can proceed to [set up KDK](index.md).
...
Using a directory path with a space in it causes the install to fail (for example, using the default home directory of the Windows user, where you've possibly used your full name). This is [an open issue](https://github.com/khulnasoft/khulnasoft-development-kit/-/issues/1380).

Additionally, using a directory path in Windows file system mount points (such as `/mnt/c` or `/mnt/d`) [results in permissions issues](https://khulnasoft.com/khulnasoft-org/khulnasoft/-/issues/347422#note_757891300). Instead, you must use a path inside the WSL OS, such as inside `/home/<user>/`. If you must access the files from Windows, you can go to `\\wsl$` in Explorer to browse the file system. For example, `/home/sid/dev/kdk` would be visible at `\\wsl$\Ubuntu-20.04\home\sid\dev\kdk`.
...
Some functions (and specs) require a special Ruby installed with additional [patches](https://khulnasoft.com/khulnasoft-org/khulnasoft-build-images/-/tree/master/patches/ruby).
These patches are already applied when running on KhulnaSoft CI/CD or when using KhulnaSoft Compose Kit,
but since KhulnaSoft Development Kit uses `asdf` they must be manually enabled.
...
https://khulnasoft.com/khulnasoft-org/khulnasoft-build-images/-/raw/master/patches/ruby/2.7/debug-segfault.patch
https://khulnasoft.com/khulnasoft-org/khulnasoft-build-images/-/raw/master/patches/ruby/2.7/thread-memory-allocations.patch
EOF
)"
...
# Compile Ruby 3.0.4
export RUBY_APPLY_PATCHES="$(cat <<EOF
https://khulnasoft.com/khulnasoft-org/khulnasoft-build-images/-/raw/master/patches/ruby/3.0/thread-memory-allocations.patch
EOF
)"
...
After you've completed the steps on this page, [use KDK to install KhulnaSoft](index.md#use-kdk-to-install-khulnasoft).
