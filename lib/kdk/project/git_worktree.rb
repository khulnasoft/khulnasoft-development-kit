...
module KDK
  module Project
    class GitWorktree
      ...
      def update
        if conflicts?
          raise KDK::UserInteractionRequired.new(
            "Unresolved conflicts detected in #{worktree_path}. Please resolve them and then try this command again.",
            docs: 'https://docs.khulnasoft.com/user/project/merge_requests/conflicts/'
          )
        end
        ...
        with_stashed do
          ...
          auto_rebase? ? execute_rebase : execute_checkout_and_pull
        end
      end
      ...
      def execute_command(command, **args)
        ...
        sh = Shellout.new(command, chdir: worktree_path).execute(**args)

        if !sh.success? && sh.read_stderr.include?('Permission denied')
          raise KDK::UserInteractionRequired.new(
            "'#{command}' failed. Git does not have necessary permissions to fetch, please set up authentication and retry.",
            docs: 'https://docs.khulnasoft.com/user/ssh/'
          )
        end

        if !sh.success? && sh.read_stderr.include?('Your SSH key has expired')
          raise KDK::UserInteractionRequired.new(
            "'#{command}' failed. Your SSH key expired, please generate a new SSH key and add it to your KhulnaSoft account.",
            docs: 'https://docs.khulnasoft.com/user/ssh/'
          )
        end
      ...
      def conflicts?
        staged = KDK::Shellout.new(%w[git diff --cached --check], chdir: worktree_path).run
        unstaged = KDK::Shellout.new(%w[git diff --check], chdir: worktree_path).run
        !(staged.empty? && unstaged.empty?)
      end
      ...
      def parse_git_remotes(output)
        ...
        # origin  https://github.com/khulnasoft/khulnasoft-shell.git (fetch)
        # origin  https://github.com/khulnasoft/khulnasoft-shell.git (push)
