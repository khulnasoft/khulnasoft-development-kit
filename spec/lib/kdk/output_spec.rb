...
RSpec.describe KDK::Output do
  ...
  describe '.prompt' do
    ...
    context 'when we have a TTY' do
      ...
      context 'when sending an interrupt' do
        context 'with raise_interrupt: true' do
          it 'propagates the interrupt' do
            expect(Kernel).to receive(:print).with("#{message}: ")
            expect($stdout).to receive(:flush).and_call_original
            expect($stdin).to receive_message_chain(:gets, :to_s, :chomp).and_raise(Interrupt)

            expect { described_class.prompt(message, raise_interrupt: true) }.to raise_error(Interrupt)
          end
        end
        ...
        it 'returns an empty string' do
          ...
          expect(described_class.prompt(message)).to eq('')
        end
      end
