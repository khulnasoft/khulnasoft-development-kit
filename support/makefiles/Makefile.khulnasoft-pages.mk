...
khulnasoft_pages_version = $(shell support/resolve-dependency-commitish "${khulnasoft_development_root}/khulnasoft/KHULNASOFT_PAGES_VERSION")
...
ifeq ($(asdf_opt_out),false)
	...
	@echo "${DIVIDER}"
	$(Q)cd ${khulnasoft_pages_dir} && ASDF_DEFAULT_TOOL_VERSIONS_FILENAME="${khulnasoft_pages_dir}/.tool-versions" asdf install
else ifeq ($(mise_enabled),true)
	@echo
	@echo "${DIVIDER}"
	@echo "Installing mise tools from ${khulnasoft_pages_dir}/.tool-versions"
	@echo "${DIVIDER}"
	$(Q)cd ${khulnasoft_pages_dir} && mise install -y
else
	@true
endif
