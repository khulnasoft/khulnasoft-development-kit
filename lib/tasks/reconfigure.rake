...
spinner_task reconfigure:  %w[
  ...
  reconfigure:provision_license
  kdk:shell_completion
].freeze
...
namespace :reconfigure do
  ...
  desc 'Provision license'
  task :provision_license do |t|
    t.skip! unless KDK::Telemetry.team_member? && KDK.config.kdk.license_provisioning.enabled?

    KDK::Licensing.new.activate
  end
end
