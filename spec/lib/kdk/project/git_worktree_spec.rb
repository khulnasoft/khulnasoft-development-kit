  ...
  describe '#update' do
    before do
      allow(KDK::Shellout).to receive(:new)
        .with(%w[git diff --cached --check], chdir: worktree_path)
        .and_return(instance_double(KDK::Shellout, run: ""))
      allow(KDK::Shellout).to receive(:new)
        .with(%w[git diff --check], chdir: worktree_path)
        .and_return(instance_double(KDK::Shellout, run: ""))
    end
    ...
    context 'when user has misconfigured Git authentication' do
      it 'raises a KDK::UserInteractionRequired error' do
        expect_update(
          stash_result: stash_nothing_to_save,
          fetch_success: false,
          fetch_error: <<~ERROR
            git@khulnasoft.com: Permission denied (publickey).
            fatal: Could not read from remote repository.
          ERROR
        )
        expect { new_subject.update }.to raise_error(KDK::UserInteractionRequired)
      end
    end

    context 'when user has expired SSH key' do
      it 'raises a KDK::UserInteractionRequired error' do
        expect_update(
          stash_result: stash_nothing_to_save,
          fetch_success: false,
    ...
    context 'when conflicts are present' do
      before do
        allow(KDK::Shellout).to receive(:new)
          .with(%w[git diff --cached --check], chdir: worktree_path)
          .and_return(instance_double(KDK::Shellout, run: "conflict info"))
      end

      it 'raises a UserInteractionRequired error' do
        expect { new_subject.update }.to raise_error(KDK::UserInteractionRequired)
      end
    end
    ...
    def expect_update(stash_result:, fetch_success: true, fetch_error: nil)
      ...
      expect_shellout(command, success: fetch_success, stderr: fetch_error || "fetch_success: #{fetch_success}",
