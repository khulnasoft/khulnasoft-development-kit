...
RSpec.describe KDK::Command::ResetOpenbaoData do
  ...
  before do
    ...
    allow(KDK::Output).to receive(:warn).with("We're about to remove OpenBao PostgreSQL data.")
  ...
  context 'when the user accepts the prompt' do
    ...
    it 'runs' do
      ...
      drop_database_command = common_command + ['drop database khulnasofthq_development_openbao']
      expect_shellout_stream(drop_database_command)

      create_database_command = common_command + ['create database khulnasofthq_development_openbao']
