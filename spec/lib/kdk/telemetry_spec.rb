...
RSpec.describe KDK::Telemetry, :with_telemetry do
  ...
  before do
    remove_memoized_instance_variable(described_class, :@release)

    sh = kdk_shellout_double(run: git_email)
    allow_kdk_shellout_command(%w[git config --get-all user.email], chdir: KDK.config.kdk_root).and_return(sh)
    allow_kdk_shellout_command(%w[git config --get-all user.email], chdir: KDK.config.khulnasoft.dir).and_return(sh)
    sh = kdk_shellout_double(run: 'd06f00d;Wed, 17 Sep 2025 16:53:03 +0200')
    allow_kdk_shellout_command(%w[git show HEAD --format=%h;%cD --no-patch]).and_return(sh)
    ...
    sh = kdk_shellout_double(run: '13.5.1')
    allow_kdk_shellout_command(%w[sw_vers -productVersion]).and_return(sh)
    allow(Dir).to receive(:exist?).and_call_original
    allow(Dir).to receive(:exist?).with(KDK.config.kdk_root).and_return(true)
    allow(Dir).to receive(:exist?).with(KDK.config.khulnasoft.dir).and_return(true)
  end
  ...
  describe '.with_telemetry' do
    ...
    it 'tracks the finish of the command' do
      ...
      expect(client).to receive(:track).with(a_string_starting_with('Finish'), hash_including(:duration, :environment, :platform, :platform_version, :architecture, :version_manager, :team_member, :enabled_services, :session_id, :cpu_count))
    ...
    context 'when the block returns false' do
      it 'tracks the failure of the command' do
        expect(client).to receive(:identify).with('testuser')
        expect(client).to receive(:track).with(a_string_starting_with('Failed'), hash_including(:duration, :environment, :platform, :platform_version, :architecture, :version_manager, :team_member, :enabled_services, :session_id, :cpu_count))
    ...
    context 'when the block raises an error' do
      it 'tracks the failure of the command' do
        expect(client).to receive(:identify).with('testuser')
        expect(client).to receive(:track).with(a_string_starting_with('Failed'), hash_including(:duration, :environment, :platform, :platform_version, :architecture, :version_manager, :team_member, :session_id, :cpu_count))
    ...
    describe 'payload' do
      ...
      describe 'release' do
        it { expect(payload[:release]).to eq({ date_unix: 1758120783, sha: "d06f00d" }) }
      end
      ...
      describe 'platform_version' do
        ...
        before do
          allow(KDK::Machine).to receive(:macos?).and_return(macos?)
        end

        context 'when macOS' do
          let(:macos?) { true }

          it { expect(payload[:platform_version]).to eq('13.5.1') }
        end

        context 'when Linux' do
          let(:macos?) { false }

          it { expect(payload[:platform_version]).to eq('unknown') }
        end
      end
    end
  end

  describe '.send_custom_event' do
    let(:client) { double('Client') } # rubocop:todo RSpec/VerifiedDoubles

    before do
      allow(described_class).to receive_messages(telemetry_enabled?: true, client: client)
      allow(client).to receive(:track)
      allow(client).to receive(:identify)
    end

    it 'does not overwrite existing keys with custom parameters' do
      extras = { value: 'should_not_overwrite' }

      described_class.send_custom_event('test_metric', 1.23, extras: extras)

      expect(client).to have_received(:track).with('Custom test_metric', hash_including(value: 1.23, extras: extras))
    end
  end
  ...
  describe '.client' do
    ...
    before do
      ...
      remove_memoized_instance_variable(described_class, :@client)
    ...
    context 'when in CI' do
      ...
      it 'initializes the khulnasoft sdk client with the CI configuration' do
        expect(KhulnasoftSDK::Client).to receive(:new).with(
          ...
          host: described_class::CI_ANALYTICS_BASE_URL,
          buffer_size: 100
        ).and_return(mocked_client)
  ...
  describe '.init_sentry' do
    ...
    it 'initializes the sentry client with expected values' do
      ...
      expect(config).to receive_message_chain(:sdk_logger, :level=).with(Logger::WARN)
  ...
  describe '.team_member?' do
    ...
    before do
      remove_memoized_instance_variable(described_class, :@team_member)
