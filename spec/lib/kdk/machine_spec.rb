...
RSpec.describe KDK::Machine do
  ...
  describe '.platform_version' do
    context 'when macOS' do
      it 'returns the macOS version' do
        stub_macos

        expect(subject.platform_version).to eq('13.5.1')
      end

      it 'returns the macOS version: `major.minor` if patch version is not present' do
        sh = kdk_shellout_double(run: '13.5')
        allow_kdk_shellout_command(%w[sw_vers -productVersion]).and_return(sh)
        allow(subject).to receive(:macos?).and_return(true)
        expect(subject.platform_version).to eq('13.5')
      end

      it 'returns unknown when sw_vers fails' do
        allow_kdk_shellout_command(%w[sw_vers -productVersion]).and_raise(StandardError)
        expect(subject.platform_version).to eq('unknown')
      end
    end
  ...
  describe '.available_disk_space' do
    ...
    it 'returns the number of available bytes' do
      expect(described_class.available_disk_space).to be(316_383_856_000)
    end
  end
  ...
  describe '.uptime' do
    describe 'on a macOS system' do
      it 'returns uptime' do
        stub_macos
        expect(Time).to receive(:now).and_return(Time.at(1_746_548_040))
        ...
        expect(subject.uptime).to be(27_951)
      end
    end
