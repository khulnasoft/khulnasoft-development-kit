...
module KDK
  module Project
    class Base
      ...
      def with_stashed(revision: 'HEAD', &block)
        worktree = GitWorktree.new(@name, worktree_path, default_branch, revision, auto_rebase: config.kdk.auto_rebase_projects?)
        worktree.with_stashed(&block)
      end
